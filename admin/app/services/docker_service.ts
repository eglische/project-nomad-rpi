...
const PI_ARM64_IMAGE_OVERRIDES = {
...
export class DockerService {
  ...
  private hostProfilePromise?: Promise<{ architecture: string; isRaspberryPi: boolean }>
  ...
  async _createContainer(
    ...
    try {
      ...
      const arm64Target = await this._resolveArm64Target(service)
      if (arm64Target) {
      ...
      if (imageExists) {
        if (
          arm64Target &&
          !(await this._checkImageMatchesPlatform(finalImage, arm64Target.platform))
        ) {
          this._broadcast(
            service.service_name,
            'image-platform-mismatch',
            `Docker image ${finalImage} exists locally with the wrong platform. Refreshing it for ${arm64Target.platform}...`
          )
          ...
          this._broadcast(
            service.service_name,
            'image-exists',
            `Docker image ${finalImage} already exists locally. Skipping pull...`
          )
        }
      } else {
  ...
  private async _runPreinstallActions__KiwixServe(): Promise<void> {
    ...
    const WIKIPEDIA_ZIM_URL =
      'https://github.com/eglische/project-nomad-rpi/raw/refs/heads/main/install/wikipedia_en_100_mini_2025-06.zim'
  ...
  async updateContainer(
    ...
    try {
      ...
      if (await this._resolveArm64Target(service)) {
        return {
          success: false,
          message: `Updates for ${serviceName} on arm64 are managed through Nomad's Pi-specific image override. Reinstall the service after updating Nomad's pinned version.`,
        }
      }
      if (this.activeInstallations.has(serviceName)) {
  ...
  private async _resolveArm64Target(
    service: Service
  ): Promise<{ image: string; platform: 'linux/arm64' } | null> {
    const hostProfile = await this._getHostProfile()
    if (hostProfile.architecture !== 'arm64' || !hostProfile.isRaspberryPi) {
      return null
    }

    const config = PI_ARM64_IMAGE_OVERRIDES[service.service_name]
    return {
      image: config?.image || service.container_image,
      platform: 'linux/arm64',
    }
  }

  private async _getHostProfile(): Promise<{ architecture: string; isRaspberryPi: boolean }> {
    if (!this.hostProfilePromise) {
      this.hostProfilePromise = (async () => {
        const architecture = os.arch()
        const model = await this._readHostModel()
        return {
          architecture,
          isRaspberryPi: model.toLowerCase().includes('raspberry pi'),
        }
      })()
    }
  ...
  private async _readHostModel(): Promise<string> {
  ...
  private async _pullImage(
    ...
    const pullStream = await this.docker.pull(imageName, platform ? { platform } : undefined)
    const platformSuffix = platform ? ` for ${platform}` : ''
    this._broadcast(serviceName, 'pulling', `Pulling Docker image ${imageName}${platformSuffix}...`)
    await new Promise((res) => this.docker.modem.followProgress(pullStream, res))
  }

  private async _checkImageMatchesPlatform(imageName: string, platform: string): Promise<boolean> {
    try {
      const image = this.docker.getImage(imageName)
      const inspectData = await image.inspect()
      const [expectedOs, expectedArch] = platform.split('/')
      return inspectData.Os === expectedOs && inspectData.Architecture === expectedArch
    } catch (error) {
      logger.warn(`Error checking image platform for ${imageName}: ${error.message}`)
      return false
    }
  }

  private async _removeLocalImage(imageName: string): Promise<void> {
    try {
      await this.docker.getImage(imageName).remove({ force: true })
    } catch (error) {
      logger.warn(`Error removing local image ${imageName}: ${error.message}`)
    }
  }
}
