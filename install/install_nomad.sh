...
print_runtime_preflight_report() {
  ...
  if [[ -d /mnt/nomad-data ]]; then
    echo "Nomad data disk free: $(df -Ph /mnt/nomad-data | awk 'NR==2{print $4 " of " $2}')"
  fi
...
run_runtime_preflight_checks() {
  ...
  if [[ -d /mnt/nomad-data ]]; then
    ...
    nomad_data_available_kib="$(df -Pk /mnt/nomad-data | awk 'NR==2{print $4}')"
    if [[ -n "${nomad_data_available_kib}" && "${nomad_data_available_kib}" -lt 52428800 ]]; then
      record_warning "Less than 50 GiB free on /mnt/nomad-data. Large knowledgebase ingestion may stall later."
    fi
  else
    record_warning "/mnt/nomad-data is not mounted. Storage split for the knowledgebase is not ready yet."
  fi
